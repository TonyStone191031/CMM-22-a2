...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            // update pos = pos + ...
            V3D linear_movement = heading * V3D(vSideways*dt, 0, vForward*dt);  // this is y-up world frame!
            pos = pos + linear_movement;

            // update headingAngle = headingAngle + ...
            headingAngle += turningSpeed * dt;
